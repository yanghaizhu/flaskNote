...
function renderCalendar(date = new Date(), create = false) {
  ...
  if (create) {
    ...
    calendarData.forEach((item) => {
      ...
      const liClass = `date flex-center ${isActive ? 'active' : isToday? 'active' : ''} ${item.isCurrentMonth ? '' : 'light'}`;
    ...
    calendarData.forEach((item, index) => {
      ...
      isToday ? addClassNames(children[index], 'active') : isActive ? addClassNames(children[index], 'active') : removeClassNames(children[index], 'active');
